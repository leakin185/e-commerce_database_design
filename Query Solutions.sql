...
--  Part a) Never purchased by some users 
----> for every user, can have a list of the products they bought, 
----> and thus a list of products they never bought can be generated by GlobalSet - AboveTable (in SQL, prob something like SELECT * FROM XXX EXCEPT (abovetable))
----> aggregate the list above for every user, this gives us list of products that have never been purchased by at least one user 
--**actually, does "some" mean "at least one" or "more than one"?

----> for every product, as long as there it exists in the list above, it fufils the criteria for a)
